...
[/BODY]
[/PAGE 14]
(the infographic briefly lived on its own dedicated page (see PAGE 14.5's retirement note below) but was asked to move back here instead, directly under the "...understanding of both." paragraph, with "The present volume..." moved to sit beneath it rather than above — per explicit request. That reunites this page's full original prose (107w/3p) around the plate; sized the plate at 52% to keep the page inside budget with all that text present — smaller than the 83% it briefly held on its own page, since that size specifically depended on having almost no surrounding text. This is the trade-off of the requested layout: the image can be a page to itself at ~83%, or it can sit directly under the "both." paragraph the way it does now, but not both at once.)
...
[/BODY]
[/PAGE 14.5]
(this page used to be pages/chapter-2-deriving-the-optimal-strategy.html and carried the infographic itself — now retired/renamed to chapter-2-relieving-the-reader.html and holding only the two paragraphs that used to follow the plate, since the plate and its lead-in paragraph moved back to PAGE 14. No image on this page at all now: 47w/2p, quite sparse (allowed to stay that way for now — a candidate to pull more content into later, not an oversight). "Having established that strategy..." — which used to open PAGE 15 — is NOT here; it stays merged into PAGE 15's own opening paragraph, see below.)
...
[/BODY]
[/PAGE 15]
(back to 190w/4p now that "This Manual relieves the reader..." moved back to PAGE 14.5 instead of staying merged in here — comfortably clear of the ceiling. Still has its own added paragraph break before "Once the objective..." from an earlier pass, purely a readability split, no words moved for that one. Last page in the chapter's current run.)

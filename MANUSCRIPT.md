...
[/BODY]
[/PAGE 14]
(the "present volume / practical deployment" paragraph is reunited here as it originally stood — the earlier PAGE 13/14 sentence-split that separated it is no longer needed now that PAGE 13 has its own plate to fill the page. "Having established that strategy, the second volume shall concern itself with its practical application throughout the ordinary circumstances of life." — the second sentence of what used to close this page — moved on to PAGE 15 instead; see the note below for why. The infographic went through three sizes in a row: 47% (word/image-budget formula only) left a large empty gap per a real screenshot; scaled up from that screenshot's own proportions to 65%, which then clipped the last line of the final paragraph against the folio per a second screenshot; backed off to 55% to actually clear it with a margin of safety. Two lessons from getting this one wrong twice: the formula alone is too unreliable to trust either direction (too small AND too large, on the same plate), and a scale-factor estimate from one screenshot still needs a safety margin, not just a "just barely fits" target, since the real render can't be checked before pushing.)
...
[/BODY]
[/PAGE 15]
(PAGE 13-15 reflowed as one pool once the pull quote became a full plate: PAGE 13 keeps its first sentence plus PAGE 12's original four "Naturally... deduction" paragraphs whole (114w/5p) against its new plate at 53% width. PAGE 14 gets the "purpose is threefold" and reunited "present volume/practical deployment" paragraphs back in front of its own infographic, sized per the note above. The infographic's second follow-on sentence ("Having established that strategy...") moved on to PAGE 15 to keep PAGE 14 inside budget; PAGE 15 had ample room to receive it (190w/4p, well clear of the 210-word ceiling) and got its own added paragraph break before "Once the objective..." on top of that, purely a readability split — no words moved.)

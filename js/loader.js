...
/* Folio numbers are computed from position, never hand-typed — printed
   ...
   line on the next load. Every page counts straight through — 1, 2, 3…
   — title page and contents included; there's no separate roman-numeral
   front matter. The numeral itself is wrapped in its own .folio-n span
   so typography.css can give just that digit the same dashed "ruled
   blank" treatment as the jump badge — the visual cue that it,
   specifically, is what you tap to edit. */
function numberFolios(pages){
  const total = pages.length;
  pages.forEach((page, i) => {
    const n = i + 1;
    const label = `<span class="folio" data-page="${n}"><span class="folio-n">${n}</span> of ${total}</span>`;
    page.html = page.html.replace('<span class="folio" data-num="arabic"></span>', label);
  });
  return pages;
}
